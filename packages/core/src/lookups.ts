...
export async function gather(client: NansenClient, address: string, opts: GatherOptions = {}): Promise<Lookups> {
  ...
  if (!tokenHit && !burn) {
    ...
    if (transactions.ok) {
      ...
      // Non-null assertion, not `?? []`: splitDirection's own `inbound` filter (line 54) already requires a
      // non-null, non-empty tokens_received on every row it returns, so this row can never have a null
      // tokens_received — a `?? []` fallback here would be a dead branch no test could ever legitimately hit.
