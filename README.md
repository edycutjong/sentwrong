...
| Tests / CI | vitest (197 tests: unit, 40,000-case property, key-boundary), Playwright (5 suites, no key), offline fixture replay, 7-stage GitHub Actions pipeline (gates → Vercel production deploy) + CodeQL + gitleaks + Dependabot | `.github/workflows/` |
...
| Metric | Value |
|---|---|
| Tests | **197 vitest tests** (`npm test`), 9 s — **11 regression tests named after the defect they pin**, 8 key-boundary tests, 4 property-based tests |
...
| 5 | `npm test` — 197 vitest tests | 9 s |
...
| Unit testing | vitest — 197 tests, v8 coverage | ✅ |
