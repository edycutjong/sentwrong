 ...
 * The API key lives only in this process. Spend guard (lib/guard.ts admit(), shared with /q): 429 past the per-IP rate, 503 past the daily
...
export async function POST(req: NextRequest) {
  ...
  if (!gate.ok) return Response.json({ type: "error", message: gate.message }, { status: gate.status, headers: { "retry-after": String(gate.retryAfter), "cache-control": "no-store" } });

  const enc = new TextEncoder();
  const stream = new ReadableStream({
    async start(controller) {
      // a reader that went away (tab closed, a new run aborted this one) must not throw out of the stream: the verdict
      // still completes and its spend is still recorded
