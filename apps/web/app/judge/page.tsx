...
export default function Judge() {
  return (
    <div className="judge">
      <p className="hook">For the judge</p>
      <p className="sub claim">Sent crypto to the wrong address? Paste it. Nansen tells you which of four recovery routes you&apos;re on — and drafts the ticket.</p>

      <h2 className="small">The 30-second path — no setup, no key, no clone</h2>
      <ol>
        <li>
          Open{" "}
          <a href={`${APP}/q/${HERO}`}>
        ...
        <li>
          Go to <a href={APP}>{APP}</a>, click the <b>Binance deposit address</b> example, press <b>Where did it go?</b> and watch the ten Nansen calls land one row at a time, then open the provenance
          drawer: endpoint · fields · credits · ms for every call.
        </li>
      </ol>
      <p className="sub">
        Every verdict above is computed live on the server from Nansen&apos;s API when you open the link (the key stays on the server). Expect 4–13 s on a cold Vercel instance — Nansen&apos;s per-call
        latency varies by the minute.
      </p>

      <h2 className="small">Receipts — real runs, not estimates</h2>
      <table className="receipt">
        <tbody>
          ...
          <tr>
            <td>Tests</td>
            <td>
              <b>188 vitest tests</b> in 9 s, of which <b>11 regression tests named after the defect they pin</b> (e.g. &ldquo;a transactions timeout read as &lsquo;nothing on record&rsquo;&rdquo;),
          ...
          <tr>
            ...
            <td>
              The server key never appears in the verdict JSON, the streamed provenance, an error message or the fixtures; every malformed address is rejected with zero network calls. Backed by 8 unit
              tests and 4 Playwright suites that run with no key at all.
            </td>
          </tr>
          <tr>
      ...
      <h2 className="small">Reproduce — the real path</h2>
...
npm run sentwrong -- ${HERO} --explain`}</pre>
      ...
      <p className="sub">
        <b>CI / deterministic replay (not the product):</b> <code>npm run verify</code> replays the 13 recorded fixtures with <code>NANSEN_OFFLINE=1</code> — zero network, zero credits, every decision
        hash must match. It exists so CI and a reader without a key can watch the engine decide; the CLI and the web app hit Nansen live by default.
      </p>

      <h2 className="small">Honest limitations</h2>
      <ol>
        <li>
          Nansen&apos;s per-call latency varies 0.3–3 s by the minute, so a cold verdict takes 4–13 s. On Vercel the 24 h cache is in-memory per function instance — a cold start is fully live. Locally
