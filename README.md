...
| Tests / CI | vitest (214 tests: unit, 40,000-case property, key-boundary), Playwright (6 suites, no key), offline fixture replay, 7-stage GitHub Actions pipeline (gates → Vercel production deploy) + CodeQL + gitleaks + Dependabot | `.github/workflows/` |
...
| Metric | Value |
|---|---|
| Tests | **214 vitest tests** (`npm test`), 9 s — **11 regression tests named after the defect they pin**, 8 key-boundary tests, 4 property-based tests |
...
| 5 | `npm test` — 214 vitest tests | 9 s |
...
| Unit testing | vitest — 214 tests, 100% v8 coverage (statements/branches/functions/lines) | ✅ |
